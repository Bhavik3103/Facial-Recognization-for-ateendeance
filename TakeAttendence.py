...
from mtcnn.mtcnn import MTCNN   #multi task cascadeing convulation nueral network
from sklearn.metrics import accuracy_score
from keras_facenet import FaceNet
from sklearn.preprocessing import Normalizer, LabelEncoder

# Loading the FaceNet() model for Face Recognition based Attendance System
embedder = FaceNet()
...
def find_face(img, img_size=(160, 160)):
    ...
    faces = detector.detect_faces(img) # To detect the face using mtcnn 
    if faces:
        x, y, w, h = faces[0]['box']
        x, y = abs(x), abs(y)
        face = img[y:y+h, x:x+w] # Creating the Bounding box 
        # converting it to image object to resize it
...
def embed(face):
    ...
    face = np.expand_dims(face, axis=0) # 2D array is formed as embeddings requires 2D array
...
def mark_attendance(name, roll):
    ...
    if roll not in roll_list:
        att1 = pd.DataFrame({'Name': [name], 'Roll': [roll], 'Time': [
        ...
        # here we are just checking the time difference between previous timestamp and current time is more than 5 minites or not 
...
# To open the web cam and mark the attendence - Face Recognition based Attendance System
...
while 1:
    ...
    if i > 10:
        ...
        if face is not None:
            face_emb = embed(face)
            pred = ML_model.predict(face_emb.reshape(1, -1)) # to make the 2D Array 
            name = str(id2name(pred[0]))
            if name:
                mark_attendance(name.split('-')[0], name.split('-')[1])
                cv2.rectangle(frame, (x, y), (x+w, y+h), (178, 88, 239), 1)
                cv2.putText(frame, name, (x, y-10), cv2.FONT_HERSHEY_TRIPLEX,
                            0.8, (178, 88, 239), 1, cv2.LINE_AA)
